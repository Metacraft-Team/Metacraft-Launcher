...
export default function FirstStep ({
  ...
  function filePathDisplay () {
    ...
    if (fileName.length >= 25) { fileName = `...${fileName.slice(fileName.length - 25)}` }
    const joinedPath = path.join(filePath, fileName)

    if (joinedPath.length >= 45) { return `...${joinedPath.slice(joinedPath.length - 45)}` }
    return joinedPath
  }
